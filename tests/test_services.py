...
class TestFilterEligibleCampaigns:
    ...
    def test_returns_false_when_player_does_not_match_criteria(
            self, mock_check_criteria, mock_check_active):
        # Arrange
        player = MagicMock()
        player.has_campaign.return_value = False
...
class TestCheckPlayerMatchesCampaignCriteria:
    ...
    def test_combined_requirements(self):
        ...
        # Reset country and test with another failing condition (required
        # items)
        player.country = "US"
        player.has_all_items.return_value = False
        result = check_player_matches_campaign_criteria(player, campaign)
        ...
        # Reset required items and test with another failing condition
        # (excluded items)
        player.has_all_items.return_value = True
        player.has_any_items.return_value = True
        result = check_player_matches_campaign_criteria(player, campaign)
...
class TestCheckCampaignIsActive:
    ...
    def test_returns_false_when_current_time_before_start_date(
            self, mock_datetime):
        # Arrange
    ...
    @patch('app.services.datetime')
    def test_returns_false_when_current_time_after_end_date(
            self, mock_datetime):
        # Arrange
    ...
    @patch('app.services.datetime')
    def test_returns_true_when_current_time_within_date_range(
            self, mock_datetime):
        # Arrange
